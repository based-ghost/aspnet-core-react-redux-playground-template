...
class SignalRService {
    ...
    private registerOnServerEvents(): void {
        this._hubConnection.on(_signalrConfig.LOGIN_USER_EVENT, () => {
            setTimeout(() => {
                toast.info(renderToastifyMsg('A user has logged in (SignalR)', _signalrConfig.TOASTIFY_ICON));
            }, _signalrConfig.HUB_MESSAGE_DELAY);
        });

        this._hubConnection.on(_signalrConfig.LOGOUT_USER_EVENT, () => {
            setTimeout(() => {
                toast.info(renderToastifyMsg('A user has logged out (SignalR)', _signalrConfig.TOASTIFY_ICON));
            }, _signalrConfig.HUB_MESSAGE_DELAY);
        });
        ...
        this._hubConnection.on(_signalrConfig.CLOSE_EVENT, (reason: string) => {
            this._hubConnection.stop().then(() => {
                setTimeout(() => {
                    toast.info(renderToastifyMsg(`All hub connections closed (SignalR) - ${reason}`, _signalrConfig.TOASTIFY_ICON));
                }, _signalrConfig.HUB_MESSAGE_DELAY);
            });
        });
    }
